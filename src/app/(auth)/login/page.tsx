// =============================================================================
// Login Page — Simple, Correct OAuth
// =============================================================================
// Uses next-auth/react signIn() which:
// 1. Fetches CSRF token from /api/auth/csrf (same-origin, with credentials)
// 2. POSTs to /api/auth/signin/google (same-origin, with credentials)
// 3. Gets back the Google OAuth URL
// 4. Sets window.location.href = googleUrl (full-page navigation, no CORS)
//
// The cookie config in auth/config.ts uses non-prefixed cookie names
// (authjs.csrf-token instead of __Host-authjs.csrf-token) so the CSRF
// cookie is reliably sent with fetch() requests.
//
// DO NOT change this to:
// - A server action (redirect to external URL = CORS via fetch)
// - A native <form> POST (CSRF session mismatch)
// - signIn("google", { redirect: false }) (tries fetch to Google = CORS)
// =============================================================================
...
function LoginForm() {
  ...
  const handleGoogleSignIn = () => {
    setIsLoading("google");
    // redirect: true (default) — does window.location.href, not fetch
